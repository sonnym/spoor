...
module.exports.write_user_data = function(key, user_data, cb) {
  this.read_user_data(function(err, read_user_data) {
    if (!read_user_data) { read_user_data = {} };
    read_user_data[key] = user_data;

    fs.writeFile(user_data_path, JSON.stringify(read_user_data), function(err) {
      if (err) {
