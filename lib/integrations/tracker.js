...
opts.show = function(opts) {
  opts.option("story_id", { "position": 0
                          , "help": "ID of the Tracker story to be estimated"
                          , "required": true
                          });
};
commands.show = function(story_id) {
  error_catcher(pivotal.getStory, [project_id, story_id], function(story_res) {
...
commands.estimate = function(story_id, estimate) {
  update_story(story_id, { "estimate": estimate }, function(res) {
...
commands.comment = function(story_id, comment) {
  error_catcher(pivotal.addStoryComment, [project_id, story_id, comment], function(res) {
...
opts.schedule = function(opts) {
  opts.option("story_id", { "position": 0
                          , "help": "ID of the Tracker story to be estimated"
                          , "required": true
                          });
};
commands.schedule = function(story_id) {
  schedule_story(story_id, function() {
...
_.each(["start", "finish", "deliver", "accept", "unstart"], function(verb) {
  opts[verb] = function(opts) {
    opts.option("story_id", { "position": 0
                            , "help": "ID of the Tracker story to be estimated"
                            , "required": true
                            });
  };
  commands[verb] = function(story_id) {
    update_story(story_id, { current_state: participle }, function(res) {
...
function display_story(story) {
  ...
  var comments = story.notes ? ensure_array(story.notes.note) : null;
  if (comments) {
  ...
  var tasks = story.tasks ? ensure_array(story.tasks.task) : null;
  if (tasks) {
  ...
  var attachments = story.attachments ? ensure_array(story.attachments.attachment) : null;
  if (attachments) {
